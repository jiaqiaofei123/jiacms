    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('home.statement.destroy')
                    <button class="layui-btn layui-btn-sm layui-btn-danger" id="listDelete">删 除</button>
                @endcan
                @can('home.statement.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.statement.create') }}">添 加</a>
                @endcan
                ...
                <div class="layui-input-inline">
                    <select name="author"  id="author" lay-search lay-verify="required">
                        ...
                        @foreach($author as $member)
                            <option value="{{ $member['author'] }}"  >{{ $member['member']['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="layui-input-inline">
        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,height: 500
                    ,url: "{{ route('admin.statement.data') }}" //数据接口
                    ,page: true //开启分页
                    ,cols: [[ //表头
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除当前项吗？', function(index){
                            $.post("{{ route('admin.statement.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认删除勾选项吗？', function(index){
                            $.post("{{ route('admin.statement.destroy') }}",{_method:'delete',ids:ids},function (result) {
                                if (result.code==0){
                                    dataTable.reload()
                                }
                                layer.close(index);
